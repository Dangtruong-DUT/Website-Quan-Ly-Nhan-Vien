...
@WebServlet(urlPatterns = {"/user-department"})
		...
		Department model =FormUtil.toModel(Department.class, req);
		 ...
		 if (model.getType().equals(SystemConstant.LIST)) {
			 IPageble Pageble = new PageRequest(model.getPage(), model.getMaxPageItem(),
				...
				view ="/views/user/Department/list.jsp";
		 } else if (model.getType().equals("listEmployee")) {
			 if (model.getIdpb()!=null) {
				 view = "";
				 System.out.println(model.getIdpb());
				 resp.sendRedirect(String.format("user?type=list&idpb=%s&page=1&maxPageItem=10", model.getIdpb()));
			 }	 
		}
